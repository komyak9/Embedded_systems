...
void play_animation() {
    for (int i = 0; i < 10; i++) {
        if (i < 6)
        {
            ...
            HAL_GPIO_WritePin(GPIOD, leds_num[i + 2], GPIO_PIN_SET);
        }
        else
        {
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i], GPIO_PIN_SET);
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i + 1], GPIO_PIN_SET);
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i + 2], GPIO_PIN_SET);
        }
        ...
        if (i < 6)
        {
            ...
            HAL_GPIO_WritePin(GPIOD, leds_num[i + 2], GPIO_PIN_RESET);
        }
        else
        {
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i], GPIO_PIN_RESET);
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i + 1], GPIO_PIN_RESET);
            HAL_GPIO_WritePin(GPIOD, leds_num[10 - i + 2], GPIO_PIN_RESET);
        }
